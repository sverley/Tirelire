...
- **Deux agents par besoin, et l'auditeur passe en premier (D68).** La confiance envers le codeur
  ...
  - **Si le besoin est une règle**, le codeur la code : avec la garde et son harnais
    (`gardes.test.mjs`) quand elle en appelle une, ou sans garde, par la seule prose — une décision,
    un renommage (#107, #109). Dans les deux cas, l'auditeur code l'**amorçage** qui juge ce travail :
    un amorçage vérifie le codage d'un besoin organisationnel, qu'une garde ait été codée ou non
    (D65, D72).
