/**
 * Harnais d'audit de #71 (carnet #38, #58), écrit par la session d'audit.
 *
...
/** Les issues du lot : une dette laissée sous l'une d'elles est une dette non réglée. */
const LOT = new Set(['38', '71']);
...
/** Une dette laissée sous une issue du lot : #71 est censée la régler. */
const detteDuLot = (e) =>
  e.aBatir.filter((a) => [...a.matchAll(/#(\d+)/g)].some((m) => LOT.has(m[1])));
...
function garderOuRenoncer(texte, ids = SANS_HARNAIS) {
  ...
  for (const id of ids) {
    ...
    for (const dette of detteDuLot(e)) manquants.push(`${id} · dette laissée en place · À bâtir · ${dette}`);
  }
...
function gestesFixesEtMesures(texte) {
  ...
  for (const dette of detteDuLot(e)) manquants.push(`I6 · dette laissée en place · À bâtir · ${dette}`);
