/**
 * Harnais d'audit de #73 (#38, #58), écrit par la session d'audit.
 *
...
describe('[niveau 1] harnais de la garde', () => {
  ...
  function verifierAucuneGarantieMuette(id, texteDuRegistre, sourceDuFichier) {
    ...
    const muets = [...analyserTests(sourceDuFichier).actifs].filter(
      // Une suite qui ne porte qu'un niveau (#232) ne garde rien par elle-même.
      (nom) => !/^témoin rouge/i.test(nom) && !/^\[niveau \d\] harnais d(?:u registre|e la garde)$/.test(nom) && !new RegExp(`^${id} ·`).test(nom) && !nommes.has(nom),
    );
    assert.deepEqual(
      muets,
      [],
