/**
 * Harnais d'audit de #70 (carnet #38, #58), écrit par la session d'audit.
 *
...
/** Les issues du lot : une dette nouvelle citerait la sienne, et se verrait. */
const LOT = new Set(['13', '15', '38', '39', '70']);
...
function detteReglee(texte, ids = PORTENT_UN_PARCOURS) {
  ...
  for (const e of entreesDesUsages(texte, ids)) {
    for (const a of e.aBatir) {
      if ([...a.matchAll(/#(\d+)/g)].some((m) => LOT.has(m[1]))) restes.push(`${e.id} · À bâtir · ${a}`);
    }
  }
