...
function withHistory(): Ledger {
  ...
  months.forEach((d, i) => {
    const op: Operation = { id: `op-alim-${i}`, accountId: 'acc-pivot', origin: 'imported', date: d, label: 'SUPERMARCHE', normalizedLabel: 'SUPERMARCHE', amount: euros(-amounts[i]!), state: 'reconciled' };
    const al: Allocation = { id: `al-alim-${i}`, operationId: op.id, categoryId: 'cat-alim', envelopeId: 'env-alim', share: { kind: 'fixed', amount: op.amount } };
    l.operations.push(op);
    l.allocations.push(al);
  });
  l.operations.push({ id: 'op-frigo', accountId: 'acc-pivot', origin: 'imported', date: '2026-06-10', label: 'FRIGO', normalizedLabel: 'FRIGO', amount: euros(-600), state: 'reconciled', oneOff: true });
  l.allocations.push({ id: 'al-frigo', operationId: 'op-frigo', categoryId: 'cat-alim', envelopeId: 'env-alim', share: { kind: 'fixed', amount: euros(-600) } });
  ...
  l.operations.push({ id: 'op-tf-2025', accountId: 'acc-pivot', origin: 'imported', date: '2025-10-16', label: 'DGFIP TAXE FONCIERE', normalizedLabel: 'DGFIP TAXE FONCIERE', amount: euros(-1260), state: 'reconciled', plannedFlowId: 'flow-tf' });
  l.allocations.push({ id: 'al-tf-2025', operationId: 'op-tf-2025', categoryId: 'cat-logement', envelopeId: 'env-tf', share: { kind: 'fixed', amount: euros(-1260) } });
  return l;
}
